...
ax.legend(fontsize=9, loc='upper left', bbox_to_anchor=(1.02, 1), frameon=False)
ax.set_ylim(0, 1.1)
...
for i in range(2):
    for j in range(2):
        ax.text(j, i, f'{cm[i, j]}\n{labels[i][j]}', ha='center', va='center', fontsize=10, fontweight='bold', color='white' if cm[i, j] > 90 else 'black')
ax.set_xticks([0, 1])
ax.set_yticks([0, 1])
...
for i, (vec, val) in enumerate(zip(eigvecs.T, eigvals[idx])):
    ...
    ax1.arrow(mean[0], mean[1], vec_scaled[0], vec_scaled[1], head_width=0.2, head_length=0.3,
              fc='#e74c3c' if i == 0 else '#2ecc71', ec='#e74c3c' if i == 0 else '#2ecc71', linewidth=2.5)
