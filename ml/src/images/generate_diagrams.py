...
val_of = 1.0 - 0.2 * np.log(epochs) + 0.004 * (epochs - 10)**1.5 + 0.05 * np.random.randn(50)
...
ax2.text(18, 0.1, 'Early stopping', ha='center', fontsize=8, color='gray', fontstyle='italic')
...
y_score = np.concatenate([np.random.beta(8, 2, 100), np.random.beta(2, 8, 100)])
...
ax.plot(fpr, tpr, color='#3498db', linewidth=2, label='ROC curve (AUC = 0.92)')
...
for i in range(4):
    ...
    # Simple decision stump approximation
...
ax1.text(25, data_drift.max()+1, "Shift", ha='center', color='red', fontsize=9)
...
ax2.text(30, 0.9, 'Boundary\nshift', ha='center', color='red', fontsize=9)
